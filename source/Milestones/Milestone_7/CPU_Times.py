...
from ODEs.Cauchy_Problem import Cauchy_Problem#, Cauchy_problem_LP, Cauchy_Problem_GBS, Cauchy_problem_RK4_emb
from ODEs.Temporal_Schemes import Euler, GBS_solution_NL, Crank_Nicolson, RK4, ERK,Leap_Frog
...
def CPU_Times():
    ...
    for i in range(len(scheme)):
        ...
        if  scheme[i] == GBS_solution_NL:
            sol = Cauchy_Problem(t, scheme[i], Arenstorf, U0, 2)
        else:
   ...
   # colores = ['#' + ''.join([random.choice('0123456789ABCDEF') for j in range(6)]) for i in range(len(scheme))]
    ...
    scheme_labels = ['Crank_Nicolson','Euler','RK4','GBS {2 mallas}','Leap-Frog','Runge-Kutta Embebido']
